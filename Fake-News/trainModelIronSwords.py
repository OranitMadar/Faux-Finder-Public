...
df = pd.read_csv("Iron_Swords_1000_Claims_Dataset.csv")

x = df["text"].values      # עמודת המשפטים
y = df["label"].values     # עמודת התוויות (0 או 1)
...
vectorization = TextVectorization(
    ...
    output_sequence_length=100,  # או אפילו 150
)
...
while True:
    sentence = input("📥 כתוב טענה (או 'exit' כדי לצאת):\n> ")
    if sentence.lower() == "exit":
        print("להתראות 👋")
        break

    # ממיר לקלט המתאים למודל
    input_tensor = tf.convert_to_tensor([sentence])
    prediction = model.predict(input_tensor)[0][0]

    label = "✅ אמיתי" if prediction >= 0.5 else "❌ פייק"
    confidence = prediction if prediction >= 0.5 else 1 - prediction

    print(f"\n🔎 תוצאה: {label} (בביטחון של {confidence:.2%})\n")
